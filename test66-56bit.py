...
while True:
    try:
        while int(end_keyspace, 16) <= int('3ffffffffffffffff', 16):
            # Generate a random 56-bit increment
            increment = ''.join(random.choices('0123456789abcdef', k=14))  # Generate a random 14-character hexadecimal value
            print("Random Increment:", increment)  # Display the randomly chosen increment
            start_keyspace = start_keyspace[:-14] + increment  # Append the increment to the right side of the start keyspace
