...
def load_checkpoint():
    if os.path.exists('checkpoint.pkl'):
        ...
        return '80000000000000000', '80100000000000000'
...
def run_VBCrLinux(start_keyspace, end_keyspace):
    ...
    command = f'./VBCrLinux -t 16 -gpu -g 544,256 -gpuId 0 -begr {start_keyspace} -endr {end_keyspace} -o {output_filename} -drk 1 -dis 1 -r 30000 -b 1MVDYgVaSN6iKKEsbzRUAYFrYJadLYZvvZ'
...
while True:
    try:
        ...
        # Delete the checkpoint file when start keyspace begins with '1xxxxxxx'
        if start_keyspace.startswith('1'):
            delete_checkpoint()
            start_keyspace, end_keyspace = load_checkpoint()
        else:
            break
